...
def upgrade() -> None:
    ...
    op.create_table("user_cards", sa.Column("id", uuid, primary_key=True), sa.Column("user_id", uuid, sa.ForeignKey("users.id", ondelete="RESTRICT"), nullable=False), sa.Column("product_id", uuid, sa.ForeignKey("card_products.id", ondelete="RESTRICT"), nullable=False), sa.Column("provider_card_id", sa.String(128), unique=True), sa.Column("status", sa.String(32), nullable=False, server_default="issuing"), sa.Column("masked_pan", sa.String(32)), sa.Column("last_four", sa.String(4)), sa.Column("balance_minor", sa.BigInteger()), sa.Column("currency", sa.String(3), nullable=False, server_default="USD"), sa.Column("scale", sa.Integer(), nullable=False, server_default="2"), sa.Column("cardholder_id", sa.String(128)), sa.Column("issue_order_id", sa.String(128), unique=True), *common)
