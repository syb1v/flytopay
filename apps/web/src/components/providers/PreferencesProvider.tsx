...
type PreferencesContextValue = {
  ...
  setCurrency: (currency: Preferences["display_currency"]) => void;
};
...
export function PreferencesProvider({ children }: { children: React.ReactNode }) {
  const [preferences, setPreferences] = useState<Preferences>(() => {
    ...
    try {
      return { ...fallback, ...JSON.parse(localStorage.getItem("flytopay.preferences") || "{}") };
    } catch {
      return fallback;
    }
  });
  ...
  useEffect(() => {
    localStorage.setItem("flytopay.preferences", JSON.stringify(preferences));
  }, [preferences]);
  const value = useMemo(
    () => ({
      preferences,
      authReady,
      setLanguage: (language: Preferences["language"]) => {
        const next = { ...preferences, language };
        setPreferences(next);
        updatePreferences({ language }).catch(() => undefined);
      },
      setCurrency: (display_currency: Preferences["display_currency"]) => {
        const next = { ...preferences, display_currency };
        setPreferences(next);
        updatePreferences({ display_currency }).catch(() => undefined);
      },
    }),
